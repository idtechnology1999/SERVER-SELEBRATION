...
const VALID_STAGES = ['free', 'fish', 'shark', 'whale'];

// Public — landing page course list
router.get('/public', asyncHandler(async (_req, res: Response) => {
  const courses = await Course.find({ status: 'active' })
    .select('title description thumbnail whatYouLearn stages')
    .lean();
  res.json({
    success: true,
    data: courses.map(c => ({
...
// POST /api/courses — create course, 4 stages auto-generated by model default
router.post('/', asyncHandler(async (req: AuthRequest, res: Response) => {
  const { title, description, thumbnail, whatYouLearn } = req.body;
  if (!title) {
    res.status(400).json({ success: false, message: 'Title is required' });
    return;
  }

  const course = await Course.create({ title, description, thumbnail, whatYouLearn });
...
// PUT /api/courses/:id — update course details (not stages)
router.put('/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  const { title, description, thumbnail, whatYouLearn, status } = req.body;
  const course = await Course.findByIdAndUpdate(
    req.params.id,
    { $set: { title, description, thumbnail, whatYouLearn, status } },
...
router.put('/:id/stages/:stage/videos/:videoId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const { stage, videoId } = req.params;
  const course = await Course.findById(req.params.id);
  if (!course) { res.status(404).json({ success: false, message: 'Course not found' }); return; }
